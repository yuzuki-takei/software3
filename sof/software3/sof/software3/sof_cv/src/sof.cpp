...
void absdiff(const cv::Mat &src, const cv::Mat &dst)
{
  ...
  // 画像の暗くする度合いを設定
  double alpha = 0.1; // 0.0から1.0の範囲で設定（1.0で元の明るさ、0.0で完全に暗くなる）
...
void absdiff1(const cv::Mat &src, const cv::Mat &dst)
{
  ...
  // 分割したチャンネルごとに差分を出す
  cv::absdiff(LmatPlanes[0], RmatPlanes[0], diff0);
  cv::absdiff(LmatPlanes[1], RmatPlanes[1], diff1);
  cv::absdiff(LmatPlanes[2], RmatPlanes[2], diff2);*/
  ...
  cv::medianBlur(dilationMat, dilationMat, 5);
  cv::imshow("dilationMat", dilationMat);
  cv::waitKey(0);
